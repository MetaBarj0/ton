...
git submodule update --init --recursive || { echo could not initialize third-parties && exit 1; }
...
# You may have to setup alternative depending on the compiler version you installed.
# It works well with clang-16 and clang-20.
# Probably also with in-between versions (though not tested)
...
if [ "$with_artifacts" = true ]; then
  ...
  cp build/storage/storage-daemon/storage-daemon build/storage/storage-daemon/storage-daemon-cli \
    ...
    artifacts
  test $? -eq 0 || {
    echo "Can't copy final binaries"
    exit 1
  }
